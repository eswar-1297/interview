...
export default function AptitudeTest({ user, onSubmit }) {
  ...
  useEffect(() => {
    ...
    push();
    const onPop = () => {
      if (!submitted) { setShowWarning(true); push(); }
    };
    const onUnload = (e) => {
  ...
  const danger        = timeLeft <= 120; // red when ≤ 2 min
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl max-w-sm w-full p-6 shadow-xl">
            ...
            <p className="text-sm text-gray-500 mb-5">
              Your progress ({answeredCount}/{questions.length} answered) will be submitted automatically.
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 h-14 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3 min-w-0">
            ...
            <span className="text-gray-400 text-xs hidden sm:inline truncate">
              {user?.name} &nbsp;·&nbsp; {answeredCount} of {questions.length} answered
            </span>
          </div>

          {/* Right: camera + timer + submit */}
          <div className="flex items-center gap-3 flex-shrink-0">
            ...
            <button
              onClick={() => {
                if (window.confirm(`Submit the test now? You've answered ${answeredCount} of ${questions.length} questions.`))
                  handleSubmit();
              }}
              className="text-xs font-semibold text-white bg-gray-900 hover:bg-gray-800 px-3 py-1.5 rounded-lg transition-colors"
            >
              Submit
            </button>
          </div>
        ...
        <div className="flex-1 min-w-0">
          ...
          {current && (
            <>
              ...
              {/* Question text */}
              <h2 className="text-base sm:text-lg font-semibold text-gray-900 leading-snug mb-6">
                {current.question}
              </h2>
              ...
              <div className="flex items-center justify-between mt-8 pt-4 border-t border-gray-100">
                ...
                {!isLast ? (
                  ...
                  <button
                    ...
                    className="flex items-center gap-1.5 text-sm font-semibold text-white bg-gray-900 hover:bg-gray-800 px-4 py-2 rounded-lg transition-colors"
                  >
                    Submit Test →
                  </button>
                )}
          ...
          <div className="sticky top-20 space-y-5">
            ...
            <div className="bg-gray-50 rounded-lg p-4 space-y-2 text-xs">
              ...
              <div className="w-full bg-gray-200 rounded-full h-1.5">
                <div className="bg-gray-900 h-1.5 rounded-full transition-all duration-300" style={{ width: `${pct}%` }} />
              </div>
